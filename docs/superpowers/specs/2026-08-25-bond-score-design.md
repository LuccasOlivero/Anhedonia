...
**Cold start:** no calendar day is treated as "missed" — for decay purposes — before the pet has hatched. `lib/pet-engine.ts`'s `LIFE_STAGE_DAYS.egg` (2 days) makes care actions physically unavailable during the egg stage (`ActionButtons` isn't even rendered), so decaying a score the user had no way to protect would be broken, not just harsh. Concretely: the first sync ever for a pet (`last_bond_sync_date is null`) initializes `last_bond_sync_date` to today with `bond_score`/`bond_streak_days` left at their defaults (0) and applies no decay for any days between `pets.created_at` and now — decay only ever evaluates days from the pet's hatch date onward.
